...
#' # Load GLGC data ####
#' ***
GLGC_data = fread(SumStats_TC_women, header=TRUE, sep="\t")
n0 = dim(GLGC_data)[1]
head(GLGC_data)
...
save(GLGC_data, file = "../temp/02_SNP_03_GLGC_women.RData")
load("../temp/02_SNP_03_GLGC_women.RData")
...
UKB_CAD = fread(SumStats_CAD_UKB_women)
...
SNPList[, Aragam_SE := Aragam$female_se]
SNPList[, Aragam_tval := Aragam$female_z]
SNPList[, Aragam_pval := Aragam$female_p_value]
SNPList[, Aragam_sampleSize := Aragam$female_n_samples]
...
save(SNPList,file = "../results/02_SNP_03_SNPListWomen.RData")
...
write.table(unique(SNPList$rsID),file = paste0(data_QC,"/02_SNP_03_SNPListWomen.txt"), 
