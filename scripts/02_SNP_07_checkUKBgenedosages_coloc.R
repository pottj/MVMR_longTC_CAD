#' ---
#' title: "Check UKB genetic data for colocalizing loci"
...
pvar1 = NewPvar(paste0(data_QC2, '02_SNP_06_UKB_TC_GLGC_coloc.pvar'))
pgen = NewPgen(paste0(data_QC2,'02_SNP_06_UKB_TC_GLGC_coloc.pgen'), pvar=pvar1)
pvar = fread(paste0(data_QC2,'02_SNP_06_UKB_TC_GLGC_coloc.pvar'))
psam = fread(paste0(data_QC2,'02_SNP_06_UKB_TC_GLGC_coloc.psam'))
...
myTab7[sex==0,sex:=2]
matched = match(psam$IID,myTab7$ID)
myTab7 = myTab7[matched,]
filt = !is.na(myTab7$ID) & myTab7$sex == psam$SEX
table(filt)
psam = psam[filt,]
geno_mat = geno_mat[filt,]
...
#' # Save data ####
#' ***
save(pvar, psam, geno_mat, file = paste0(data_QC,"02_SNP_07_UKB_TC_GLGC_coloc_merged.RData"))
save(LDTab, file = paste0("../results/02_SNP_07_LD_coloc.RData"))
