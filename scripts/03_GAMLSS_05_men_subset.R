#' ---
#' title: "Get association of SNPs on TC (men - subset)"
...
load(paste0(data_QC,"/02_SNP_04_UKB_TC_GLGC_men_merged.RData"))
...
names(data1)[15:24] = gsub("_","",names(data1)[15:24])
...
dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
  ...
  mod2 = gamlss(TC ~ myG*exposure_age + Array +
                  PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + random(x = as.factor(ID)),   
                sigma.formula = ~ myG + exposure_age + Array, 
...
save(myAssocs_X,file=paste0("../results/03_GAMLSS_05_men_subset.RData"))
