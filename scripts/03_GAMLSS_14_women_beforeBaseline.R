#' ---
#' title: "Get association of SNPs on TC (women - coloc before baseline)"
...
dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
  ...
  res1 = data.table(SNP = rep(pvar[i,ID],1),
                    model = c("women_colocBeforeBaseline"),
                    sampleSize = c(n1),
...
save(myAssocs_X,file=paste0("../results/03_GAMLSS_14_women_colocBeforeBaseline.RData"))
