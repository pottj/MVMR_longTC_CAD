#' ---
#' title: "Get association of SNPs on TC (post - coloc before baseline)"
...
load(paste0(data_QC,"/02_SNP_07_UKB_TC_GLGC_coloc_merged.RData"))
...
#' # Get effects ####
#' ***
myTab7 = myTab7[group=="postmenopausal",]
myTab6 = myTab6[BSU_ID %in% myTab7$ID,]

counter = seq(1,dim(myTab7)[1],500)
for(i in 1:dim(myTab7)[1]){
  if(i %in% counter)message("Working on i=",i)
  myID = myTab7[i,ID]
  myAge = myTab7[i,age]
  myTab6 = myTab6[!(BSU_ID==myID & exposure_age>myAge),]
}

matched = match(myTab6$BSU_ID,myTab7$ID)
myTab_long = cbind(myTab6,myTab7[matched,c(2,10:19,27)])
...
dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
  ...
  res1 = data.table(SNP = rep(pvar[i,ID],1),
                    model = c("post_colocBeforeBaseline"),
                    sampleSize = c(n1),
...
save(myAssocs_X,file=paste0("../results/03_GAMLSS_15_post_colocBeforeBaseline.RData"))
