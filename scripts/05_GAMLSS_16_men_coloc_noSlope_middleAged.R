#' ---
#' title: "Get association of SNPs on TC (men - coloc)"
...
dumTab2 = foreach(i = 1:length(mySNPs))%dorng%{
  ...
  res1 = data.table(SNP = rep(pvar[i,ID],1),
                    model = c("men_coloc_noSlope_middleAged"),
                    sampleSize = c(n1),
...
save(myAssocs_X,file=paste0("../results/05_GAMLSS_16_men_coloc_noSlope_middleAged.RData"))
