#' ---
#' title: "MVMR - women - coloc - age-groups"
...
#' ## Exposure
load("../results/05_GAMLSS_11_post_coloc_noSlope.RData")
length(unique(myAssocs_X$SNP))
myAssocs_X_old = copy(myAssocs_X)
myAssocs_X_old[,model := "women_old"]

load("../results/05_GAMLSS_12_pre_coloc_noSlope.RData")
length(unique(myAssocs_X$SNP))
myAssocs_X_young = copy(myAssocs_X)
myAssocs_X_young[,model := "women_young"]
...
myAssocs_Y_long[type!="UKB",beta := Aragam$female_beta * (-1)]
myAssocs_Y_long[type!="UKB",SE := Aragam$female_se]
myAssocs_Y_long[type!="UKB",tval := Aragam$female_z]
myAssocs_Y_long[type!="UKB",pval := Aragam$female_p_value]
myAssocs_Y_long[type!="UKB",sampleSize := Aragam$female_n_samples]

#' ## save as temporary files
save(myAssocs_X_long,myAssocs_Y_long, file = paste0("../temp/07_MVMRInput_women_coloc_noSlope_ageGroups.RData"))
...
myFlag = "MVMR_women_ageGroups"

names(myAssocs_Y_long) = c("SNP", "phenotype","sampleSize","beta_mean","SE_mean","tval_mean","pval_mean" )
myAssocs_X_long[,phenotype := "TC_women"]
myAssocs_X_long[,dumID := "TC_women"]
...
save(MVMR_results,file = paste0("../results/07_MVMR_01_women_coloc_noSlope_ageGroups.RData"))
