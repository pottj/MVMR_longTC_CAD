#' ---
#' title: "MVMR - men - coloc - age-groups"
...
load("../results/02_SNP_04_LD_men.RData")
LDTab[,SNP2 := as.character(SNP2)]
load("../results/02_SNP_06_SNPListColoc_filtered.RData")
SNPList = copy(SNPList3)

#' ## Exposure
load("../results/05_GAMLSS_17_men_coloc_noSlope_old.RData")
length(unique(myAssocs_X$SNP))
myAssocs_X_old = copy(myAssocs_X)
myAssocs_X_old[,model := "men_old"]

load("../results/05_GAMLSS_16_men_coloc_noSlope_middleAged.RData")
length(unique(myAssocs_X$SNP))
myAssocs_X_young = copy(myAssocs_X)
myAssocs_X_young[,model := "men_young"]

myAssocs_X = rbind(myAssocs_X_old,myAssocs_X_young)
...
myAssocs_Y_UKB = myAssocs_Y_UKB[SNP %in% myAssocs_Y_long$rsID & model == "men"]
...
table(is.element(myAssocs_Y_long$rsID,Aragam$rsid_ukb))
Aragam = Aragam[rsid_ukb %in% myAssocs_Y_long$rsID,]

filt2 = Aragam$rsid_ukb == "rs10127939" & Aragam$reference_allele == "C"
Aragam = Aragam[!filt2,]

matched = match(myAssocs_Y_long[type!="UKB",rsID],Aragam$rsid_ukb)
Aragam = Aragam[matched,]
stopifnot(myAssocs_Y_long[type!="UKB",rsID] == Aragam$rsid_ukb)
myAssocs_Y_long[type!="UKB",beta := Aragam$male_beta * (-1)]
myAssocs_Y_long[type!="UKB",SE := Aragam$male_se]
myAssocs_Y_long[type!="UKB",tval := Aragam$male_z]
myAssocs_Y_long[type!="UKB",pval := Aragam$male_p_value]
myAssocs_Y_long[type!="UKB",sampleSize := Aragam$male_n_samples]

#' ## save as temporary files
save(myAssocs_X_long,myAssocs_Y_long, file = paste0("../temp/07_MVMRInput_men_coloc_noSlope_ageGroups.RData"))
...
myFlag = "MVMR_men_ageGroups"

names(myAssocs_Y_long) = c("SNP", "phenotype","sampleSize","beta_mean","SE_mean","tval_mean","pval_mean" )
myAssocs_X_long[,phenotype := "TC_men"]
myAssocs_X_long[,dumID := "TC_men"]
myAssocs_X_long[,type := paste0("TC_",model,"_",type)]
filt_SNPs = myAssocs_X_long[pval==0,SNP]
myAssocs_X_long = myAssocs_X_long[!is.element(SNP,filt_SNPs),]
myAssocs_Y_long = myAssocs_Y_long[!is.element(SNP,filt_SNPs),]
...
dumTab3 = foreach(k = 1:length(myOutcomes))%do%{
  ...
  message("Working on exposure TC and outcome ",myOutcome," ...")
  
  # do MVMRs
  MVMR0 = MVMR_jp_POPS(data_exposure = myAssocs_X_long,
                       data_outcome = myAssocs_Y2,
                       exposure_name = "TC_men", 
                       outcome_name = myOutcome,
                       flag = myFlag,
                       GX_pval_treshold = 1,
                       getPlot = F,
                       corTab = LDTab,
                       corTab_threshold = 0.1,sampleSize_GX = rep(mySampleSize,each=2),
                       random = F,getCondF = T,getUni = F)
...
save(MVMR_results,file = paste0("../results/07_MVMR_02_men_coloc_noSlope_ageGroups.RData"))
