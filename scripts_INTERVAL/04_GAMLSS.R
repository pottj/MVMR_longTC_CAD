...
load("../results/INTERVAL/03_candidateSNPlist_pgen.RData")
mySNPs = copy(pvar)
myCHR = mySNPs[,unique(CHR)]

data_QC2 = gsub("~","../../",data_QC)
psam1 = fread(paste0(data_QC2,'INTERVAL/temp_pgen/INTERVAL_chr1.psam'))
myGenoMat = matrix(data = NA, ncol = 1,nrow = dim(psam1)[1])

dumTab1 = foreach(j = 1:length(myCHR))%do%{
  #j=1
  pvar1 = NewPvar(paste0(data_QC2, 'INTERVAL/temp_pgen/INTERVAL_chr',myCHR[j],'.pvar'))
  pgen = NewPgen(paste0(data_QC2,'INTERVAL/temp_pgen/INTERVAL_chr',myCHR[j],'.pgen'), pvar=pvar1)
  pvar = fread(paste0(data_QC2,'INTERVAL/temp_pgen/INTERVAL_chr',myCHR[j],'.pvar'))
  psam = fread(paste0(data_QC2,'INTERVAL/temp_pgen/INTERVAL_chr',myCHR[j],'.psam'))
...
save(geno_mat,pvar,psam,file = paste0(data_QC2,"INTERVAL/temp_pgen/INTERVAL_merged.RData"))
...
dumTab2 = foreach(k = 1:dim(pvar)[1])%dorng%{
  ...
  dumTab3 = foreach(i = 1:length(mySettings))%do%{
    #i=1
    # load phenotype data
    data1 = fread(paste0(data_QC,"/INTERVAL/01_Prep_TrajGWAS_",mySettings[i],".csv"))
    
    # add SNP vector
    matched = match(data1$genID,psam$IID)
    ...
    data1[,ID2 := as.factor(ID)]
    data2 = copy(data1)
    data2 = data2[,c(1,2,3,8,9,10,12,14:25)]
    
    # run gamlss 
    mod2 = gamlss(CHOL ~ myG + age + age2 + med_lipid +
                    PC_1 + PC_2 + PC_3 + PC_4 + PC_5 + PC_6 + PC_7 + PC_8 + PC_9 + PC_10 + random(x = ID2),   
                  sigma.formula = ~ myG + age + age2 + med_lipid + 
  ...
  myAssocs_3 = rbindlist(dumTab3)
  myAssocs_3
}
  
myAssocs = rbindlist(dumTab2)
myAssocs

save(myAssocs,file=paste0("../results/INTERVAL/04_potentialIVs_GAMLSS.RData"))
