#' ---
#' title: "MVMR INTERVAL using trajGWAS estimates"
...
#' # Load data ####
#' ***
#' INTERVAL sample sizes per setting
load("../results/INTERVAL/01_descriptiveTable.RData")
tab1 = copy(res)
...
dumTab1 = foreach(i = 1:length(mySettings))%do%{
  ...
  if(sum(filt2)!=0){
    ...
    res4 = data.table(MR_model = rep("univariate",1),
                      exposure = c("TC mean"),
                      outcome = c(res2@Outcome),
                      NR_SNPs_type = c(dim(dataX[mean_pval<5e-8])[1]),
                      beta_IVW = c(res2@Estimate),
                      SE_IVW = c(res2@StdError),
                      pval_IVW = c(res2@Pvalue),
                      condFstat = c(res2@Fstat),
                      HeteroStat = c(res2@Heter.Stat[1]),
                      HeteroStat_pval = c(res2@Heter.Stat[2]))
...
save(MR_results, file = "../results/MVMR/03_MVMR_INTERVAL_trajGWAS.RData")
