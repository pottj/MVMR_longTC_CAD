#' ---
#' title: "MVMR UKB using GAMLSS estimates per sex"
...
dumTab = foreach(i = 1:dim(ToDoList)[1])%do%{
  ...
  stopifnot(dataX[setting == myRow$setting1,rsID] == dataY$rsid_ukb)
  stopifnot(dataX[setting == myRow$setting1,EA] == dataY$reference_allele)
  stopifnot(dataX[setting == myRow$setting2,rsID] == dataY$rsid_ukb)
  stopifnot(dataX[setting == myRow$setting2,EA] == dataY$reference_allele)
...
save(MR_results, file = "../results/MVMR/06_MVMR_UKB_gamlss_ageGroups.RData")
