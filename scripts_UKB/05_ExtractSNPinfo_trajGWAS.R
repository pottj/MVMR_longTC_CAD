...
dumTab1 = foreach(i = 1:length(mySubpops))%do%{
  ...
  save(data1,file = paste0(trajGWAS_UKB_results_WaldTest, "/SumStats_",mySetting,".RData"))
  data1
}
...
test[,table(N)]
test[N==12,]
TC_data[rsID == test[N==12,snpid]]
exposureData[snpid == test[N==12,snpid]]
exposureData = exposureData[!(snpid == test[N==12,snpid] & allele2 == "C")]
...
dumTab6 = foreach(i = 1:22)%do%{
  #i=1
  pvar1 = NewPvar(paste0(data_QC2, 'UKB/temp_pgen/UKB_chr',i,'.pvar'))
  pgen = NewPgen(paste0(data_QC2,'UKB/temp_pgen/UKB_chr',i,'.pgen'), pvar=pvar1)
  pvar = fread(paste0(data_QC2,'UKB/temp_pgen/UKB_chr',i,'.pvar'))
  psam = fread(paste0(data_QC2,'UKB/temp_pgen/UKB_chr',i,'.psam'))
...
save(pvar,file = "../results/UKB/05_candidateSNPlist_pgen.RData")
...
save(exposureData,file = "../results/UKB/05_potentialIVs_trajGWAS_Wald.RData")
